...
def configure(input_bams, gcat_conf, run_conf, sample_conf):
    ...
    for sample in sample_conf.melt:
        ...
        output_files[sample] = []
        output_files[sample].append("%s/ALU.final_comp.vcf" % (output_dir))
        output_files[sample].append("%s/HERVK.final_comp.vcf" % (output_dir))
        output_files[sample].append("%s/LINE1.final_comp.vcf" % (output_dir))
        output_files[sample].append("%s/SVA.final_comp.vcf" % (output_dir))
        arguments = {
            "SAMPLE": sample,
