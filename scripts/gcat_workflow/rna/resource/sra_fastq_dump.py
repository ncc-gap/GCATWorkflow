...
OUTPUT_FQ2_FORMAT = "sra_fastq_dump/{sample}/2_1.fasq"
...
#fastq-dump -v --split-files {RUN_ID} -O {OUTPUT_DIR}/temp
...
if [ -e {RUN_ID}.sra_2.fastq ]; then mv {RUN_ID}.sra_2.fastq 2_1.fastq; fi
if [ -e {RUN_ID}.sra.fastq ]; then mv {RUN_ID}.sra.fastq 1_1.fastq; fi

# squeeze
seqtk squeeze {OUTPUT_DIR}/temp/1_1.fastq > {OUTPUT_DIR}/1_1.fastq

if [ -e {OUTPUT_DIR}/temp/2_1.fastq ]
then
    seqtk squeeze {OUTPUT_DIR}/temp/2_1.fastq > {OUTPUT_DIR}/2_1.fastq
fi
...
def configure(gcat_conf, run_conf, sample_conf):
    ...
    for sample in sample_conf.sra_fastq_dump:
        ...
        arguments = {
            "RUN_ID": sample_conf.sra_fastq_dump[sample][1],
